    ...
    {{ groep.naam }}
      ...
      <div class="flex-grow flex justify-center items-center mb-16 mt-8">
        <div class="w-full max-w-4xl p-8 bg-white shadow-lg rounded-lg">
          <h2 class="text-3xl font-bold mb-6 text-center text-[#456A50]">Technische Competenties</h2>
          <ul v-if="technischeCompetenties.length" class="divide-y divide-gray-200">
            <li
            ...
            >
              <div>
                <div class="text-[#456A50] font-bold">{{ technischeCompetentie.naam }}</div>
                <div class="text-[#456A50]">Groep: {{ technischeCompetentie.taak?.groep?.naam }}<span class="mx-2">|</span> Taak: {{ technischeCompetentie.taak?.naam }}</div>
                <div class="text-[#456A50]">Beschrijving: <span class="italic">{{ technischeCompetentie.beschrijving }}</span></div>
              </div>
              <div class="flex space-x-4">
                <Icon
                  icon="material-symbols:edit"
                  class="text-[#456A50] hover:text-[#104116] hover:scale-110 hover:ease-in-out hover:duration-500 w-8 h-8 cursor-pointer"
                  @click="openEditModal(technischeCompetentie.technischeCompetentieID, technischeCompetentie.naam, technischeCompetentie.beschrijving, technischeCompetentie.taak?.taakID, technischeCompetentie.taak?.groep?.groepID)"
                />
                <Icon
                  ...
                  @click="openDeleteModal(technischeCompetentie.technischeCompetentieID, technischeCompetentie.naam)"
                />
              </div>
            </li>
          </ul>
...
<div v-if="isEditModalVisible" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
  <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
    <h2 class="text-2xl font-bold mb-4 text-center">Technische Competentie Bewerken</h2>
    <div class="mb-4">
 ...
 >
  ...
  >
    ...
    <div class="mb-4">
      ...
      />
      <label class="block text-gray-700 font-bold mb-2" for="editedBeschrijving">
                Technische Competentie Beschrijving:
              </label>
            <textarea
    ...
    <div class="flex justify-end space-x-4">
      ...
      <button @click="updateTechnischeCompetentie(selectedTechnischeCompetentieID, editedNaam, editedBeschrijving, editedTaakID, editedGroepID); closeModal()" class="bg-[#456A50] hover:bg-[#104116] hover:ease-in-out hover:duration-500 text-white px-4 py-2 rounded">Bijwerken</button>
    </div>
  </div>
</div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
      <h3 class="text-xl font-semibold mb-4">Weet je zeker dat je "{{ selectedTechnischeCompetentieNaam }}" wilt verwijderen?</h3>
      <div class="flex justify-end space-x-4">
  ...
  try {
    ...
    if (groepID) {
      this.taken = response.data.filter((taak: Taak) => taak.groep && taak.groep.groepID === groepID);
    } else {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:3000/technische-competentie', { headers: { Authorization: `Bearer ${token}` } })
      this.technischeCompetenties = response.data.sort((a: TechnischeCompetentie, b: TechnischeCompetentie) => a.naam.localeCompare(b.naam))
    } catch (error) {
      console.error('Er is een fout opgetreden bij het ophalen van de technische competenties:', error)
    }
  },
  async fetchTaken() {
    try {
...
async addTechnischeCompetentie() {
  try {
    console.log('Adding technische competentie with name:', this.naam, 'and taak ID:', this.selectedTaakID, 'and groep ID:', this.selectedGroepID);
    const token = localStorage.getItem('access_token');
    const response = await axios.post('http://localhost:3000/technische-competentie', 
      { naam: this.naam, beschrijving: this.beschrijving, taak: { taakID: this.selectedTaakID}, groep: { groepID: this.selectedGroepID }}, 
      { headers: { Authorization: `Bearer ${token}` } }
    );
    ...
    async confirmDelete() {
      try {
        const token = localStorage.getItem('access_token');
        await this.deleteTechnischeCompetentie(this.selectedTechnischeCompetentieID);
        this.isDeleteModalVisible = false; 
      } catch (error) {
    ...
    async deleteTechnischeCompetentie(technischeCompetentieID: number) {
      try {
        const token = localStorage.getItem('access_token');
        await axios.delete(`http://localhost:3000/technische-competentie/${technischeCompetentieID}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        this.fetchTechnischeCompetenties(); 
      } catch (error) {
        console.error('Er is een fout opgetreden bij het verwijderen van de technische competentie:', error);
      }
    },
  
    openEditModal(technischeCompetentieID: number, naam: string, beschrijving: string, taakID: number, groepID: number) {
  this.selectedTechnischeCompetentieID = technischeCompetentieID;
...
async updateTechnischeCompetentie(technischeCompetentieID: number, updatedNaam: string, updatedBeschrijving: string, updatedTaakID: number) {
  try {
    const token = localStorage.getItem('access_token');
    await axios.patch(`http://localhost:3000/technische-competentie/${technischeCompetentieID}`, 
      { 
        naam: updatedNaam,
    ...
    console.error('Error updating technische competentie:', error.response ? error.response.data : error);
  }
},
