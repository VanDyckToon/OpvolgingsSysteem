<template>
    <div class="min-h-screen bg-[#ECF3EB] flex flex-col">
      ...
      <div class="grid grid-cols-4 gap-4 pt-8 pb-2 px-14">
        <div class="col-span-3">
          <h1 class="text-[#104116] text-4xl font-extrabold pt-4">Competenties Beheren</h1>
        ...
        <div class="col-span-1">
          <img src="../assets/competentie-icoon.svg" alt="Image" class="h-32 w-auto mb-2 object-contain m-auto place-content-center">
        </div>
      </div>
      <div class="flex-grow flex justify-center items-center mb-16 mt-8">
        <div class="w-full max-w-4xl p-8 bg-white shadow-lg rounded-lg">
          <h2 class="text-3xl font-bold mb-6 text-center text-[#456A50]">Competenties</h2>
          ...
          <ul v-if="competenties.length" class="divide-y divide-gray-200">
            ...
            >
              <div>
                <div class="text-[#456A50] font-bold">{{ competentie.naam }}</div>
              ...
              <div class="flex space-x-4">
                <Icon
                  ...
                  @click="openEditModal(competentie.competentieID, competentie.naam, competentie.beschrijving)"
                />
                <Icon
                  ...
                  @click="openDeleteModal(competentie.competentieID, competentie.naam)"
                />
              </div>
            </li>
          </ul>
          <p v-else class="text-center text-gray-500">Geen competenties gevonden</p>
        </div>
        </div>
      </div>
      ...
      <div v-if="isEditModalVisible" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
          <h2 class="text-2xl font-bold mb-4 text-center">Competentie Bewerken</h2>
          <div class="mb-4">
          ...
          <div class="flex justify-end space-x-4">
            ...
            <button @click="updateCompetentie(selectedCompetentieID, editedNaam, editedBeschrijving); closeModal()" class="bg-[#456A50] hover:bg-[#104116] hover:ease-in-out hover:duration-500 text-white px-4 py-2 rounded">Bijwerken</button>          </div>
        </div>
      </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
      <h3 class="text-xl font-semibold mb-4">Weet je zeker dat je "{{ selectedCompetentieNaam }}" wilt verwijderen?</h3>
      <div class="flex justify-end space-x-4">
  ...
  async fetchCompetenties() {
    try {
      ...
      const response = await axios.get('http://localhost:3000/competentie', { headers: { Authorization: `Bearer ${token}` } })
      this.competenties = response.data.sort((a: Competentie, b: Competentie) => 
      a.naam.localeCompare(b.naam)
    );
    } catch (error) {
      console.error('Er is een fout opgetreden bij het ophalen van de competenties:', error)
    }
  },
  ...
  try {
    const token = localStorage.getItem('access_token');
    await axios.post('http://localhost:3000/competentie', 
    ...
    async deleteCompetentie(competentieID: number) {
      try {
        const token = localStorage.getItem('access_token');
        await axios.delete(`http://localhost:3000/competentie/${competentieID}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        this.fetchCompetenties(); // Refresh the list after deletion
      } catch (error) {
        console.error('Er is een fout opgetreden bij het verwijderen van de competentie:', error);
      }
    },
  ...
  async updateCompetentie(competentieID: number, updatedNaam: string, updatedBeschrijving: string) {
  try {
    const token = localStorage.getItem('access_token');
    await axios.patch(`http://localhost:3000/competentie/${competentieID}`, 
    ...
    console.error('Error updating competentie:', error.response ? error.response.data : error);
  }
},
