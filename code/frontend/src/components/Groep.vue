<template>
    <div class="min-h-screen bg-[#ECF3EB] flex flex-col">
      ...
      <div class="grid grid-cols-4 gap-4 pt-8 pb-2 px-14">
        <div class="col-span-3">
          <h1 class="text-[#104116] text-4xl font-extrabold pt-4">Groepen Beheren</h1>
        ...
        <div class="col-span-1">
          <img src="../assets/groep-icoon.svg" alt="Image" class="h-32 w-auto mb-2 object-contain m-auto place-content-center">
        </div>
      </div>
      <div class="flex-grow flex justify-center items-center mb-16 mt-8">
        <div class="w-full max-w-4xl p-8 bg-white shadow-lg rounded-lg">
          <h2 class="text-3xl font-bold mb-6 text-center text-[#456A50]">Groepen</h2>
      ...
      <div v-if="isEditModalVisible" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
          ...
          <div class="flex justify-end space-x-4">
            ...
            <button @click="updateGroep(selectedGroepID, editedNaam); closeModal()" class="bg-[#456A50] hover:bg-[#104116] hover:ease-in-out hover:duration-500 text-white px-4 py-2 rounded">Bijwerken</button>          </div>
        </div>
      </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
      <h3 class="text-xl font-semibold mb-4">Weet je zeker dat je "{{ selectedGroepNaam }}" wilt verwijderen?</h3>
      <div class="flex justify-end space-x-4">
  ...
  async fetchGroepen() {
    try {
      ...
      const response = await axios.get('http://localhost:3000/groep', { headers: { Authorization: `Bearer ${token}` } })
  ...
  try {
    const token = localStorage.getItem('access_token');
    await axios.post('http://localhost:3000/groep', 
      { naam: this.naam }, 
    ...
    async deleteGroep(groepID: number) {
      try {
        ...
        console.error('Er is een fout opgetreden bij het verwijderen van de groep:', error);
      }
    },
  ...
  try {
    const token = localStorage.getItem('access_token');
    await axios.patch(`http://localhost:3000/groep/${groepID}`, 
      { naam: updatedNaam }, 
      { headers: { Authorization: `Bearer ${token}` } }
    );
    ...
    console.error('Error updating groep:', error.response ? error.response.data : error);
  }
},
