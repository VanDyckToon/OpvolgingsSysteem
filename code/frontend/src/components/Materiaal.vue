<template>
  ...
  async addMateriaal() {
    try {
      await axios.post('http://localhost:3000/materiaal', { beschrijving: this.beschrijving })
      this.beschrijving = '' // Het input veld wordt leeg gemaakt
      this.fetchMaterialen() // Laad de lijst terug in
    } catch (error) {
      console.error('Er is een fout opgetreden bij het toevoegen van het materiaal:', error)
    }
  },
  ...
  async deleteMateriaal(materiaalID: number) {
    try {
      ...
      this.fetchMaterialen() // Laad de lijst terug in na verwijderen
    } catch (error) {
  ...
  try {
    const response = await axios.patch(`http://localhost:3000/materiaal/${materiaalID}`, {
      beschrijving: updatedBeschrijving
    });
    console.log("Update response:", response);  // Kijkt voor een response
    this.isEditModalVisible = false;
    this.fetchMaterialen();  // Laad de lijst in
  } catch (error) {
